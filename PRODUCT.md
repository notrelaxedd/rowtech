...
- Belief ladder: the node measures something real (a force curve per stroke, not a guess); it is practical in a real boathouse (its own network, no app, survives the dock power-off); the crew view is where it is heading and beta crews shape it; applying costs nothing and commits to nothing. (Founder intentions in part: the site doesn't mention the dock power-off or that beta crews shape the crew view, and what the beta costs and asks of crews is still an owner placeholder on the home page, so "costs nothing and commits to nothing" isn't on the site.)
- Proof on hand: no testimonials, press or logos yet. Force and Vieve are drawn on the site as vector concept renders, after "Vieve V1 + Force, concept A" (2026-09-22), in the site's own colours: they are the design, not photographs of finished hardware, and dimensions and parts are proposed. The numbers on every screen are computed from the same stroke model the rest of the page uses, not typed in. The home page lists what beta crews get: testing units, for now, and a direct line to the people building it; price and what happens if a unit fails are owner placeholders, and the beta's terms are on `/terms`. After applying, the form says we read every application and get in touch by email.
